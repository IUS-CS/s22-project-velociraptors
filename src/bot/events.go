...
const (
	...
	challengeMessage5 = "\n🟥 = Abstain"
	challengeMessage6 = "\n✋  = Close Voting"
	helpMessage       = "Vinny is a bot that lets you call out your friends!\n\n-Reply '!challenge' to a message to initiate a challenge.\n-To vote on a challenge, select the reaction square with the color that corresponds with your pick. You can only vote once per challenge.\n-To change your vote, remove your original reaction vote and add a new one.\n-The vote ends when two people select the 'Close Voting' reaction."
...
func MessageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if strings.EqualFold(messageContent, commandHelp) {
	...
	if strings.EqualFold(messageContent, "!reply") {
		_, err := s.ChannelMessageSendReply(m.ChannelID, "reply!", m.Reference())
		if err != nil {
			oops(err, "channelMessageSendReply")
		}
	}
	...
	if strings.EqualFold(messageContent, commandChallenge) && messageType == discordgo.MessageTypeReply {
		...
		debate := "\n\n<@" + referencedAuthorID + ">" + " says: `" + m.ReferencedMessage.ContentWithMentionsReplaced() + "`\n\n<@" + authorUserID + "> disagrees!\n"
...
func MessageReactionCreate(s *discordgo.Session, r *discordgo.MessageReactionAdd) {
	...
	if reactionEmoji == "✋" {
		...
		if !hasVotedStop(db, userVotingRecord) {
			fmt.Println("has not voted stop and added vote")
		...
		if stopVotesTotal == 2 {
			...
			if winnerID(challengeEntry) == "tie" {
				_, err := s.ChannelMessageSendReply(r.ChannelID, tie, message.Reference())
				if err != nil {
					oops(err, "ChannelMessageSend")
					return
				}
			...
			if winnerID(challengeEntry) == challengeEntry.ChallengerID {
				_, err := s.ChannelMessageSendReply(r.ChannelID, winnerIsChallenger, message.Reference())
				if err != nil {
					oops(err, "ChannelMessageSend")
					return
				}
			...
			if winnerID(challengeEntry) == challengeEntry.DefenderID {
				_, err := s.ChannelMessageSendReply(r.ChannelID, winnerIsDefender, message.Reference())
				if err != nil {
					oops(err, "ChannelMessageSend")
					return
				}
...
func MessageReactionDelete(s *discordgo.Session, r *discordgo.MessageReactionRemove) {
	...
	if reactionEmoji == "✋" {
		...
		if hasVotedStop(db, userVotingRecord) {
			fmt.Println("has voted stop and removed vote")
